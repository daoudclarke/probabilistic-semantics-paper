...
builders to build a model for $T$ and a model for $T$ and $H$
together, and used the size of these models as an indication of the
degree of entailment, the reasoning being that if the model for $T$
and $H$ together is larger than just the model for $T$, then $H$
contains a lot of knowledge which is not in $T$. This idea can be
related to our approach, by assuming probability distribution over
models which depends only on the size of the model, and makes larger
models less likely.
...
work. In the full paper, we will investigate what features of natural
language semantics can be learnt from toy data.
