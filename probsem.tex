...
john & likes & mary & 1 & 0 & 1 & 1\\
mary & likes & john & 1 & 1 & 1 & 1\\
mary & likes & mary & 1 & 0 & 1 & 1\\
...
probability distribution over the set of models: we assume that each
of these four models has probability 0.25, and all other models have
probability zero.
...
interpretations are assumed to be generated by a random process.

% For each interpretation $\langle E, F\rangle$, let $F_\tau$ denote the
% set of all possible denotations associated with meaningful expressions
% of type $\tau$, i.e.
% $$F_\tau = \{F(\gamma) : \gamma \in \Lambda_\tau\}$$


We assume a fixed set $E$, and that $F$ is randomly generated, with
probabilities that are conditional on a hidden variable. Let $H$
denote the set of possible hidden values. We assume the following
process to generate $F$:
\begin{itemize}
