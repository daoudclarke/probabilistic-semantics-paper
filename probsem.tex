...
text, for example, we may try and compute entailment between two
sentences both containing the word ``bank'', one in the context of
finance and one in the context of rivers, in which case we would like
...
sacrifice some nice theoretical properties satisified by the first
approach. A nice consequence of this approach, however, is that
...
As may be apparent, our formalism does not make use of any explicit
...
The extra flexibility granted by the stochastic semantics formalism results
...
The two models also have different computational properties:
stochastic semantics allows for efficient computation with dynamic
...
In recent work, Cooper et al.~\cite{Cooper:14}, in a development on
earlier work \cite{Eijck:12} have described a probabilistic semantics for natural language in which probability is distributed over situation types~\cite{Barwise:83}. They propose a rich type system in which the judgment about whether a given situation is of a given type is probabilistic and present a compositional interpretation function that assigns probability-conditions rather than truth-conditions to sentences. The work of Cooper et al shares similar motivations to ours. In particular, it aims to account for the gradience of semantic judgements (for example, gradations of entailment) and stresses the importance of providing an account of semantic learning within language acquisition.

Their approach differs from ours in a number of key respects. 
\begin{enumerate}
\item they model probabilistic semantics not over worlds but over situations - claim to be more tractable?
\item they define a probabilistic interpretation function - we don't - we define a generative process over models so that probabilities of meaning are defined in terms of probability distribution over worlds
\item Defining probability over situation types rather than possible worlds (models) 
\item They describe an approach to semantic learning which makes use
  of observations of situations. Our approach is to learn from pairs
  of sentences and truth values directly.
\item The result of their learning is an assignment of probabilities
  to situations; the result of our learning is the ability to
  determine degrees of entailment between pairs of sentences.
\item Our approach can be related back to distributional semantics: as
  output of our learning procedure we can extract lexical
