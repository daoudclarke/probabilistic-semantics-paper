...
An example is shown in Table ref{table:pmsat}.
...
$\land$ and $\lor$ of type $(t/t)/t$ and $\lnot$ of type
$t/t$. For example $\lor (P_1)(\lnot (P_2))$ is a sentence of this language. Using a more familiar and suggestive notation it might be written as $P_1 \lor \lnot P_2$. We fix a generative model for interpretations of the language as follows. 
...
in $\{\top, \bot\}$ with equal probability, while the symbols $\land$, $\lor$ and $\lnot$
simply reproduce the familiar
logical truth functions for conjunction, disjunction and negation respectively, with probability 1. Note for example that there is an interpretation for which $\lor (P_1)(\lnot (P_2))$ has value $\top$. On the other hand, the sentence $\land (P_1)(\lnot (P_1))$ will have value $\bot$ for all interpretations.
