...
% % {\bf UPDATE THIS LIST??}
The success of vector-space models is due to several factors. They support fine-grained judgements of similarity, allowing us to account for semantic gradience, for example, that the lexeme {\em pear} is more similar to {\em banana}  than to, say, {\em cat}. Moreover, distributional vectors can be learned from corpus data by counting occurrences of distributional contexts for a word or phrase.  Alternatively, neural network architectures may be applied to acquire vector-based representations \cite{Socher:12,Mikolov:13}. 
...
This paper considers an alternative  strategy for developing a computational semantics. Starting with a compositional, model-theoretic semantics, this is augmented by work in probabilistic semantics \cite{Gaifman:64,Nilsson:86,Sato:95}. Probabilistic semantics provides a rich seam of ideas that can be applied to the construction of a compositional semantics with desirable properties such as gradience and learnability. Below, we explore two different ways in which this might be achieved. Our objective is to map out some of the territory,  to consider issues of expressiveness and computational complexity and to provide useful guidance for others considering venturing into this landscape. 
...
describing the probability of theories. The probability of $T$ is
$\mu(M(T))$. The conditional
probability of $s_1$ given $s_2$ is interpreted as the {\em degree to which $s_2$ entails $s_1$\/} and defined by:
...
formulation both occurrences of \emph{bank} are forced to have the same denotation, but
a word's meaning should be allowed
...
associate a set of models with a sentence and instead assume that pairs of sentences and
...
extraction. A learning system such as those we have described can be
