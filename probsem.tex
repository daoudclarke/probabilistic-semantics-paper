...
complexity. Both approaches provide a compositional semantics and account for the gradience of semantic judgements of belief and inference. \footnote{The authors are grateful for the comments of a number of anonymous reviewers. The work described here was funded as part of EPSRC EP/I037458/1, {\em A Unified Model of Compositional and Distributional Semantics: Theory and Applications\/.}}
...
language for propositional logic with symbols $P_i$ of type $t$,
$\land$ of type $(t/t)/t$ and $\lnot$ of type
$t/t$. For example $\land (P_1)(\lnot (P_2))$ is a sentence of this language. Using a more familiar and suggestive notation it might be written as $P_1 \land \lnot P_2$. We fix a generative model for interpretations of the language as follows. 
Hidden values are not needed, so assume $H$ has a single
element. 
%Denotations and the probabilities of generating them are
%shown in Table \ref{table:pmsat}.  
Further assume distributions defined by $\theta$  such that  the $P_i$ take values
in $\{\top, \bot\}$ with equal probability, while the symbols $\land$ and $\lnot$
simply reproduce the familiar
logical truth functions for conjunction and negation with probability 1. Note that as a consequence there must be an interpretation for which $\land (P_1)(\lnot (P_2))$ has value $\top$. In contrast, a sentence such as $\land (P_1)(\lnot (P_1))$ must have value $\bot$ for all interpretations.

It follows from the construction above that a sentence has truth value $\top$ with non-zero probability if and only if it is satisfiable. Hence we can solve
SAT if we can solve PM-SAT, and so PM-SAT is NP-hard. Finally, to show NP-completeness we note that PM-SAT can be solved by a non-deterministic machine in
