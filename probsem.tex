...
\abstract{
  ...
  compositional approaches. This paper argues for the adoption of probabilistic semantics as a
  basis for such a unifying account. We present two different approaches combining probabilistic semantics with Montague semantics. The first approach extends model-theoretic semantics by assuming a probability distribution over models. The second account .... MORE ... consider an approach for learning semantics directly from data.
   }
% \end{quote}
%\end{abstract}
...
\item We propose a framework for natural language semantics which
  ...
  specifying a probability distribution over Montague-style models. \hide{As
...
\item We describe ongoing work to construct a probabilistic Montague
  ...
  \item We show that restricting the space of models makes learning feasible, whilst
  retaining many of the desirable properties of full Montague
  semantics. We show that taking account of lexical ambiguity leads to a computationally more tractable formulation of the learning problem.
\end{enumerate}
...
Recently, a number of researchers have begun to address the
...
\item It maintains the tradition of intensional approaches to semantics,
...
In the following we consider two different ways of constructing a probabilistic extension of Montague semantics. We demonstrate their properties on
simple natural language examples and show that it is feasible to learn semantics from text by restricting the space of models appropriately. In the first approach we define a probability distribution over models in terms of  a random generation process over
interpretations. Whilst this provides a straightforward means of constructing a probabilistic semantics, it carries a restrictive assumption that in interpreting any given text, the meaning of all words is fixed in advance. That is, whilst the meaning of words may vary between different interpretations, for any given interpretation they are fixed. Clearly, this is unrealistic as it is easy to construct examples where the interpretation of a given word varies across the text. Our second approach removes this restriction and allows for ambiguity. This is consistent with distributional accounts of  lexical semantics, where the meaning of a polysemous word is identified with the combination of all of its distributional contexts. A surprising consequence of allowing for ambiguity is that learning semantics from text becomes more tractable. We present an implementation within the context of Sato's approach to the semantics of probabilistic logic programs. 
...
which proposes a probabilistic type theory. 
...
In this section we describe our first model of a probabilistic
...
conditions of the sentences that make it up. Note that probability
...
We verified that our system correctly predicted the reversal of
entailment: pairs which we expected to entail were given a degree of
entailment of 1.0 by our system, and pairs we expect not to entail
were given a degree of entailment of 0.5. We did not see any
difference between the two systems on these small examples.
...
related. Indeed, Sato semantics is also based around a probability
distribution over models, however because of its foundation in logic
programming, only \emph{Herbrand} models are considered. The semantics
of the switches used in the PRISM language are that they are assigned
a random value on each evaluation when determining the truth of a
statement. This is what allows a term to have different meanings
...
The extra flexibility granted by the Sato semantic formalism results
in the loss of certain nice properties. For example, in the
probabilistic Montague semantics formalism, it is easy to see that the
degree to which any sentence entails itself is necessarily 1. This is
no longer guaranteed in our instantiation of Sato semantics. Indeed
this seems to be a necessary consequence of incorporating statistical
ambiguity into the formalism. In standard model-theoretic semantics,
sentences are typically disambiguated before any reasoning is
performed. However, if we do not perform disambiguation, one cannot
say that a sentence entails itself, since it may be disambiguated
differently in different contexts. Thus it seems that if we accept
ambiguity as an inherent feature of language, it makes sense to
sacrifice some properties which are expected when handling logical
expressions.

The two models also have different computational properties. The Sato
semantic formalism allows for efficient computation with dynamic
...
In recent work, Cooper et al.~\cite{Cooper:14} have described a probabilistic semantics for natural language in which probability is distributed over situation types~\cite{Barwise:83}. They propose a rich type system in which the judgment about whether a given situation is of a given type is probabilistic and present a compositional interpretation function that assigns probability-conditions rather than truth-conditions to sentences. The work of Cooper et al shares similar motivations to ours. In particular, it aims to account for the gradience of semantic judgements (for example, gradations of entailment) and stresses the importance of providing an account of semantic learning within language acquisition.

Their approach differs from ours in a number of key respects. 

1) they model probabilistic semantics not over worlds but over situations - claim to be more tractable?
2) they define a probabilistic interpretation function - we don't - we define a generative process over models so that probabilities of meaning are defined in terms of probability distribution over worlds
3) 


Defining probability over situation types rather than possible worlds (models) 
